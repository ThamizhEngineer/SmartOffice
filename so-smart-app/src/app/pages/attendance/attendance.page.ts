...
import { AuthUser } from './../../vo/authUser.object';
...
export class AttendancePage implements OnInit {
  ...
  constructor(public toastCtrl: ToastController, public alertController: AlertController, private router: Router, private attendanceService: AttendanceService,
    private modalCtrl: ModalController, public locationAccuracy: LocationAccuracy,
    private geolocation: Geolocation, private androidPermissions: AndroidPermissions,
    private nativeGeocoder: NativeGeocoder, private utilsService: UtilsService) { }

  ngOnInit() {
   this.todayDate = this.utilsService.formatDate(new Date());
  ...
  checkCurrentStatus(){
    this.attendanceService.getMyAttendance().subscribe((x:any[]) => {
      console.log(x)
      if(x.length!=0){
        x.forEach(element => {
          var attendanceRecord = element;
          if (attendanceRecord.attendanceDt==this.todayDate) {
            if (attendanceRecord.checkOut) {
              this.ifAlreadyCheckedOut(attendanceRecord);
            }else{
              this.ifAlreadyCheckedIn(attendanceRecord);
            }
          }
        });
      }
  ...
  ifAlreadyCheckedIn(attendanceRecord){
    console.log("Already Checked In")
    this.authUser.attendanceId=attendanceRecord.id;
    this.disableCheckInButton=true
    this.checkInTime=attendanceRecord.startDt
  ...
  ifAlreadyCheckedOut(attendanceRecord){
    console.log("Already Checked Out")
    this.authUser.attendanceId=attendanceRecord.id;
    this.disableCheckInButton=true
    this.checkInTime=attendanceRecord.startDt
    this.checkOutTime=attendanceRecord.endDt
    this.stopwatch = attendanceRecord.totalTimeLogged + " hours";
  }
  ...
  startCheckInLoc(){
    this.utilsService.presentLoader().then((x)=>{
      this.authUser.logType='check-in'
      var time = new Date()
      this.authUser.checkIn=time.toTimeString().substring(0,8);
  ...
  checkInLocation() {
    this.attendanceService.checkin(this.authUser).subscribe(x => {
      ...
      let message = this.utilsService.getErrorStatus(err.status);
      this.utilsService.dismissLoader();
  ...
  checkOutInit() {
    this.loadMap();
    var time = new Date()
    this.authUser.checkOut=time.toTimeString().substring(0,8);
    this.authUser.logType='check-out'
    this.attendanceService.checkOut(this.authUser).subscribe(x => {
