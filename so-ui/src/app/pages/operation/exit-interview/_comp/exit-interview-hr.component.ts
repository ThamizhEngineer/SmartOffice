...
@Component({
    selector: 'exit-interview-hr',
    templateUrl: './exit-interview-hr.component.html'
})
export class ExitInterviewHrComponent implements OnInit{
    ...
    formIsCondition(){
     ...
     update(){
        this.formBeforSave();
        this.exitInterviewService.hrUpdateExitInterview(this.exitInterview.id,this.exitInterview).subscribe(x=>{
            this.msg = { type: 'success', text: "Exit Clearance Updated Successfully"}
        },error=>{
            this.msg = { type: 'danger', text: "Error While Updating Exit Clearance"}
        })
     }
