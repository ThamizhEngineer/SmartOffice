...
export class VacancyRequestService {
    ...
    getVacancyrequests(){
        return this.http.get(environment.serviceApiUrl + 'transaction/vacancy-requests/dir-view', this.commonService.jwt()).map((response: Response) => response).map(res => res.json()); 
    }
