...
export class AttendanceService {
    ...
    checkin(coordinates) {
		...
		if (currentUser) {
			...
			return this.http.post(this.baseUrl + 'locations/check-in', obj, this.commonService.jwt()).map((response: Response) => response.json());
		}
    }
	...
	checkOut(coordinates) {
		...
		if (currentUser) {
			let obj = JSON.stringify({ latitude: coordinates.latitude.toString(), longitude: coordinates.longitude.toString(), appToken: currentUser.appToken, appClientId: this.clientId });
			return this.http.post(this.baseUrl + 'locations/check-out', obj, this.commonService.jwt()).map((response: Response) => response.json());
		}
    }
	...
	logging(coordinates) {
		...
		if (currentUser) {
			...
			return this.http.post(this.baseUrl + 'locations/log-location', obj, this.commonService.jwt()).map((response: Response) => response.json());
		}
    }
	...
	getEmployees(){		
        return this.http.get(this.serviceApiUrl + 'master/employees', this.commonService.jwt()).map((response: Response) => response.json());
    }
	...
	bulkAttendances(arr: any) {
        return this.http.patch(this.serviceApiUrl + 'transaction/attendance/direct/bulk', arr, this.commonService.jwt()).map((response: Response) => response.json());
    }
}