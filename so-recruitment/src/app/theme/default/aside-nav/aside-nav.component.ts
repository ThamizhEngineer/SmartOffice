...
export class AsideNavComponent implements OnInit {
	...
	ngOnInit(){
		...
		this._userService.getMenu().subscribe((response:any[]) => {
			this.rows = response.length;
			this.menuData = [];
			let authRoles =[];
			if (this.currentUser.authUserRoles!= undefined){
				authRoles = this.currentUser.authUserRoles;
			}
			// build menu based in user's roles
			response.forEach(menuItem => {
					// if(menuItem.children != undefined){
					// 	menuItem.children = menuItem.children.filter(child => (child.roles!=undefined)&&(child.roles.some(r => authRoles.some(authRole => authRole.authRoleId ==r))));
					// }	
