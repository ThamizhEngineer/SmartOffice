...
@Service
public class EmployeeAcc2ManagerUpdateService {
	private static final String EMAIL_ACC2_UPDATE_TEMPLATE_NAME="html/employee-acc2-update";
	@Autowired
	SmsHelper smsHelper;
	...
	public void updateAcc2ManagerToSelectedEmp(Employee employee) {
		try {
			...
			message.setSubject("Your Hr-1 Manager has Been Changed");
			message.setFrom(commonUtils.getSenderEmail());
			message.setTo(employee.getContactEmailId());
			final String htmlContent= this.htmlTemplateEngine.process(EMAIL_ACC2_UPDATE_TEMPLATE_NAME, ctx);
